...
function AllDestination() {
    ...
    const handleFilter = async (option) => {
        const response = await axios.get(`${BASE_URL}/resorts/adminfilterdestination/${option.value}`)
        setDestinationList(response.data)
    }
    return (
        <div className='table-div'>
            <div className="resort-table-header">
                <div style={{ display: 'flex', alignItems: "center", gap: "1.5rem" }}>
                    <h1 style={{ color: "black" }}>Destination List</h1>
                    ...
                    {destinationList.map((item) => (
                        <tr>
                            ...
                            <td className='action-col' style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                <Link className='action-text' to={`/admin/view-destination/${item.id}`} ><p className='edit'><AiFillEye /> View</p></Link>
